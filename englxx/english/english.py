...
class english_asignaturas(osv.osv):
 ...
 #   edad = (datetime.now().date() - datetime.strptime(fecha_nacimiento, '%Y-%m-%d').date()).days / 365
  ...
  _columns = {
    ...
    'fecha_nacimiento':fields.date('Fecha Nacimiento', required=False),
    'edad': fields.integer('Edad',size=9, required=False),
...
#      result = {}
 #     for calificaciones in self.browse(cr, uid,ids,context=context):
 #         result[calificaciones.id] = calificaciones.primer_quimestre + calificaciones.segundo_quimestre
 #         return result
  ...
  _columns = {
    ...
    'nombre':  fields.char('Nombre', size=50, required=False),
    'nota': fields.char('nota', required=False),
    'primer_quimestre': fields.float('Primer Quimestre', digits=(3,2)),
    'segundo_quimestre': fields.float('Segundo Quimestre', digits=(3,2)),
    'promedio': fields.float('Promedio', digits=(3,2)),
#    'total': fields.funtion(_get_total, method=True, type=float string='Total', store=True),
    'total': fields.float('Total',digits=(3,2)),
 #   'primer_quimestre': fields.float('Primer Quimestre',digits_compute=dp.get_precision('Account')),
   }
